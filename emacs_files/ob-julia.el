...
\"Save FileIO\"
display(fn::String, x) = begin; mysave(fn, x); nothing; end
function display(fn::String, e::Exception)
    # assume there _is_ an extension
    err = string(\"ERROR: \", e)
    ext = lowercase(split(fn, '.')[end])
    if ext in (\"pdf\", \"svg\", \"png\", \"eps\")
      save(fn, plot(title = err));
...
\"This function is used to check whether FileIO is available. If not, try to save raw data to file\"
function mysave(fn::String, x)
    if isdefined(Main, :FileIO) && isa(getfield(Main, :FileIO), Module)
        save(fn, x)
    else
      @info \"FileIO not available, saving raw data\"
      write(fn, string(x))
    end
end
...
(defvar org-babel-julia-write-object-with-fileio-command "%s(\"%s\",%s)")
  ...
  (case result-type
    (value
     (let* ((tmp-file (or output-file (org-babel-temp-file "julia-")))
	    (trim-body (string-trim-right body))
	    (suppress (if (> (length trim-body) 1)
			  (string-equal (substring trim-body -1) ";")
			nil)))
